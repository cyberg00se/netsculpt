...
async function serializeONNXModel(model) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const mainNodes = model.nodes.filter(node => node.type !== "Input" && node.type !== "Output");
            rawGraph.node = mainNodes.map((node) => {
            ...
            const inputNodes = model.nodes.filter(node => node.type === "Input");
            rawGraph.input = inputNodes.map((node) => {
            ...
            const outputNodes = model.nodes.filter(node => node.type === "Output");
            rawGraph.output = outputNodes.map((node) => {
                const rawValueInfo = ValueInfoProto.create();
                rawValueInfo.name = node.name;
